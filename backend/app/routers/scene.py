...
from ..models import SceneObject, ObjectKind, User
...
async def create_object(payload: SceneObjectCreate, admin: User = Depends(require_admin),
...
async def delete_object(object_id: str, admin: User = Depends(require_admin),
                         db: AsyncSession = Depends(get_db)):
    obj = await db.get(SceneObject, object_id)
    if not obj:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Objekt nicht gefunden")
